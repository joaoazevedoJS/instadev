...
class SessionsController {
  ...
  public signup = async (req: Request, res: Response) => {
    ...
    const existAccount = await this._model.existAccount(
      String(email),
      user_name,
    );
    ...
    try {
      const data = await this.FactoryCreateAccountData(
        String(email),
        user_name,
        String(name),
        String(password),
  ...
  public signin = async (req: Request, res: Response) => {
    ...
    const verifyPassword = await bycript.compare(
      String(password),
      user.password,
    );
  ...
  private FactoryCreateAccountData = async (
    email: string,
    user_name: string,
    name: string,
    password: string,
