...
class UserController {
  ...
  public show = async (req: Request, res: Response) => {
    ...
    const following = await this._model.ReadWithWhereCount('following', {
      user_id: Number(id),
    });
    const followers = await this._model.ReadWithWhereCount('following', {
      following_id: Number(id),
    });
    const publications = await this._model.ReadWithWhereCount('publications', {
      user_id: Number(id),
    });
